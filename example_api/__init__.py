...
def main(global_config, **settings):
    ...
    # Generate ramses routes before we add more routes by hand so we have
    # an access to those generated by ramses.
...
def create_resources(config):
    ...
    # Get needed route from map.
    # 'markets' - collection route
    # 'market' - item route
    # Join route names by colon (':') to access nested routes.
    # E.g. in case of '/users/{user_id}/stories/{story_id}'
    # 'user:stories' - stories collection route
    # 'user:story' - stories item route
    market = root.resource_map['market']
    market.add('product', 'products',
               view='example_api.views.markets.MarketAttributesView',
               factory='nefertari.acl.GuestACL')
